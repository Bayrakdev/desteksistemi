...
var bayrakxd = ""; //botunuzun adını
var bayrakxd2 = ""; //sunucunuzun adını girin
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${bayrakxd} Talep Sistemi | ${client.guilds.size} sunucuya hizmet veriyorum! | ${prefix}yardım`);
});
...
if (message.content.toLowerCase().startsWith(prefix + `talepaç`)) {
    ...
    message.guild.createChannel(`talep-${message.author.id}`, "text").then(c => {
        ...
        const embed = new Discord.RichEmbed()
        .setColor(0xCF40FA)
        .addField(`Hey ${message.author.username}!`, ` talebini başarılı bir şekilde açtım! ${bayrakxd2} tarafından paylaşılmıştır.`)
        .setTimestamp();
        c.send({ embed: embed });
        message.delete();
    }).catch(console.error);
...
if (message.content.toLowerCase().startsWith(prefix + `talepkapat`)) {
    ...
    message.channel.send(`Destek kanalını kapatacaksın, emin misin? kapatmak için **!kapat** yazman yeterli. Unutma, cevabını **15** saniye içinde vermelisin.`)
    .then((m) => {
